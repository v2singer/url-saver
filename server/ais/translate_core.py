...
def valid_translation(text, translation):
    ...
    if (translation.startswith('"') and '"注' in translation):
        translation = translation[1:].split('"注')[0]
        return True, translation_postprocess(translation)
    ...
    if '译文' in translation and ': "' in translation and '"' in translation:
        translation = translation.split('："')[-1].split('"')[0]
        return True, translation_postprocess(translation)
...
def _translate(summary, transcript, target_language='简体中文'):
    ...
    full_translation = []
    fixed_message = [
            {'role': 'system', 'content': f'You are a expert in the field of this video.\n{info}\nTranslate the sentence into {target_language}.下面我让你来充当翻译家，你的目标是把任何语言翻译成中文，请翻译时不要带翻译腔，而是要翻译得自然、流畅、准确和地道，使用优美和高雅的表达方式。请将人工智能的“agent”翻译为“智能体”，强化学习中是`Q-Learning`而不是`Queue Learning`。数学公式写成plain text，不要使用latex。确保翻译正确和简洁，不需要返回翻译之外的内容，绝对不要添加任何注解或说明的内容。\n只返回翻译内容。'},
...
if __name__ == '__main__':
    translate_all_transcript_under_folder('downloaded', '简体中文')
